...
const MyAppointments = () => {
    ...
    // Funcion para el formato( 20_01_2000 => 20 Jan 2000 ) 
    const slotDateFormat = (slotDate) => {
        const dateArray = slotDate.split('_')
        return dateArray[0] + " " + months[Number(dateArray[1])] + " " + dateArray[2]
    }

    // Obtener user citas con API
                ...
                {appointments.map((item, index) => (
                    <div key={index} className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-4 border-b'>
                        ...
                        <div className='flex-1 text-sm text-[#5E5E5E]'>
                            ...
                            <p className=' mt-1'><span className='text-sm text-[#3C3C3C] font-medium'>Fecha & Tiempo:</span> {slotDateFormat(item.slotDate)} |  {item.slotTime}</p>
                        </div>
                        <div></div>
                        
                        <div className='flex flex-col gap-2 justify-end text-sm text-center'>
                            {/** NO SE PUEDE PAGAR AUN
                            ...
                            */}
                            ...
                            {!item.cancelled && !item.isCompleted && <button onClick={() => cancelAppointment(item._id)} className='text-[#696969] sm:min-w-48 py-2 border rounded hover:bg-red-600 hover:text-white transition-all duration-300'>Cancelar cita</button>}
